...
CUSTOM_FIELD_ID = 37 # replace with your actual custom field ID
...
# read custom field values from CSV
custom_field_values = read_custom_field_values_from_csv(INPUT_CSV_FILE)

for customer_code, custom_field_value in custom_field_values.items():
    print(f"Customer Code: {customer_code}, Custom Field Value: {custom_field_value}")
    ...
    if response.status_code == 200:
        ...
        print(response.json())
        write_result_to_csv(OUTPUT_CSV_FILE, customer_code, custom_field_value, "Success")
    else:
        print(f"Error {response.status_code}: {response.text}")
        write_result_to_csv(OUTPUT_CSV_FILE, customer_code, custom_field_value, f"Error {response.status_code}: {response.text}")
